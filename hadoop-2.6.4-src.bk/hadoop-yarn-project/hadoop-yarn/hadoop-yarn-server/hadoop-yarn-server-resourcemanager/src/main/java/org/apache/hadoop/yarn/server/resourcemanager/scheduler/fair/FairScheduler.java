...
public class FairScheduler extends
  ...
  protected synchronized void update() {
    ...
    // Recursively update effective workload for all queues
    rootQueue.updateEffectiveWorkload();
...
//    addRunningApps(applicationId);
    queue.getMetrics().submitApp(user);

    LOG.info("Accepted application " + applicationId + " from user: " + user
        + ", in queue: " + queueName + ", currently num of applications: "
        + applications.size());// + ", currently num of running applications: "+ runningApps.size()
...
//    runningApps.remove(applicationId);
  ...
  /**
   * update the effective share of each application attempt
   */
  private void updateEffectiveShare() {
	  
	  // compute phi_s(t), the number of active jobs
	  int numofApps = 0;
	  Resource clusterResource = getClusterResource();
	  Resource resourcePerAM = Resources.createResource(2048, 1);
	  for (SchedulerApplication<FSAppAttempt> app: applications.values()) {
		  FSAppAttempt attempt = app.getCurrentAppAttempt();
		  if (attempt == null) continue;
		  
		  FSLeafQueue queue = (FSLeafQueue) app.getQueue();
		  if (queue.isRunnableApp(attempt))
			  numofApps++;
	  }
	  
	  Resource AMResource = Resources.multiply(resourcePerAM, numofApps);
	  if (LOG.isDebugEnabled()) {
	      LOG.debug("the number of active jobs |phi_s| = " + numofApps
	    		  	+ " the cluster resource is " + clusterResource
	    		  	+ " the resource used for AM is" + AMResource);
	  }
	  
	  if (numofApps == 0) 
		  return;
	  Resource totalShares = Resources.createResource(0, 0);
	  for (SchedulerApplication<FSAppAttempt> app: applications.values()) {
		  // sanity check
		  FSAppAttempt attempt = app.getCurrentAppAttempt();
		  FSLeafQueue queue = (FSLeafQueue) app.getQueue();
		  if (attempt == null || queue.isNonRunnableApp(attempt))
			  continue;
		  
		  float workload = attempt.getEffectiveWorkload();
		  int numofLargerApps = 0;
		  for (SchedulerApplication<FSAppAttempt> ap: applications.values()) {
			  FSAppAttempt att = ap.getCurrentAppAttempt();
			  FSLeafQueue que = (FSLeafQueue) ap.getQueue();
			  if (att == null || que.isNonRunnableApp(att))
				  continue;
			  if (att.getEffectiveWorkload() >= workload - 0.001f)
				  numofLargerApps++;
		  }
		  
		  double share = 0;
		  if (numofLargerApps - 1 >= (1-BETA) * numofApps)
			  share = 1 / (BETA * numofApps);
		  else if (numofLargerApps < (1-BETA) * numofApps)
			  share = 0;
		  else
			  share = (numofLargerApps - (1-BETA) * numofApps) / (BETA * numofApps);

		  Resource shareResource = Resources.add(Resources.multiply(
					Resources.subtract(clusterResource, AMResource), 
						share), resourcePerAM);
		  Resources.addTo(totalShares, shareResource);
		  
		  attempt.setEffectiveShare(shareResource);
		  
		  if (LOG.isDebugEnabled()) {
		      LOG.debug("CLONE DEBUG: workload is " + workload
		    		  + " the number jobs with larger workload |phi_js| = " + numofLargerApps
		    		  + " and the share of " + attempt.getApplicationAttemptId() + " is " + share);
		  }
		  
		  // preempt containers of apps that use more resources than its effective share
		  if (Resources.greaterThan(RESOURCE_CALCULATOR, clusterResource, attempt.getCurrentConsumption(), shareResource))
			  preemptAppAttempt(attempt);
		  
	  }
	  
	  Resource residual = Resources.subtract(clusterResource, totalShares);
	  if (residual.getVirtualCores() > 0 || residual.getMemory() > 0) {
		  boolean isContinue = true;
		  FSAppAttempt attempt = null;
		  while (isContinue) {
			  int index = (int)(Math.random() * numofApps);
			  index = index == numofApps ? index - 1: index;
			  SchedulerApplication<FSAppAttempt> app = (SchedulerApplication<FSAppAttempt>) applications.values().toArray()[index];
			  attempt = app.getCurrentAppAttempt();
			  FSLeafQueue queue = (FSLeafQueue) app.getQueue();
			  if (attempt != null && queue.isRunnableApp(attempt))
				  isContinue = false;
			  
		  }
		  attempt.setEffectiveShare(Resources.add(attempt.getEffectiveShare(), residual));
		  if (LOG.isDebugEnabled()) {
		      LOG.debug("CLONE DEBUG: there is resource residual " + residual
		    		  	+ "assigned to attempt " + attempt.getApplicationAttemptId());
		  }
	  }
  ...
  /*
   * preempt an attempt to let it use resources no more than its effective share
   */
  private void preemptAppAttempt(FSAppAttempt attempt) {
	  
	  Resource toPreempt = Resources.subtract(attempt.getEffectiveShare(), attempt.getCurrentConsumption());
	  if (LOG.isDebugEnabled()) {
	      LOG.debug("CLONE DEBUG: Scheduler is going to preempt " + toPreempt + " of attempt " + attempt.getApplicationAttemptId());
	  }
	  ...
	  try {
	      ...
	      while (Resources.greaterThan(RESOURCE_CALCULATOR, clusterResource,
	          toPreempt, Resources.none())) {
	    	  RMContainer container = attempt.preemptContainerRandomly();
	        if (container == null) {
	          break;
	        } else {
	          warnOrKillContainer(container);
  ...
  private void initScheduler(Configuration conf) throws IOException {
    synchronized (this) {
      ...
      // This stores per-application running information
//      this.runningApps = new ConcurrentHashMap<ApplicationId, Integer>();
