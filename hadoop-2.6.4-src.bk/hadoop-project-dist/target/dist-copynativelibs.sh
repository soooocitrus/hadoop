BUILD_DIR="/usr/local/hadoop-2.6.4-src/hadoop-project-dist/target"
                      TAR='tar cf -'
                      UNTAR='tar xfBp -'
                      LIB_DIR="${BUILD_DIR}/native/target/usr/local/lib"
                      if [ -d ${LIB_DIR} ] ; then
                        TARGET_DIR="${BUILD_DIR}/hadoop-project-dist-2.6.4/lib/native"
                        mkdir -p ${TARGET_DIR}
                        cd ${LIB_DIR}
                      ...
                      if [ -d ${BIN_DIR} ] ; then
                        TARGET_BIN_DIR="${BUILD_DIR}/hadoop-project-dist-2.6.4/bin"
