...
public abstract class RMContainerRequestor extends RMCommunicator {
  ...
  static class ContainerRequest {
    ...
    //private LegacyTaskRuntimeEstimator work = new LegacyTaskRuntimeEstimator();

    public ContainerRequest(ContainerRequestEvent event, Priority priority,long workload) {
      this(event.getAttemptID(), event.getCapability(), event.getHosts(),
          event.getRacks(), priority, workload);
    }

    public ContainerRequest(ContainerRequestEvent event, Priority priority, long workload,
                            long requestTimeMs) {
      this(event.getAttemptID(), event.getCapability(), event.getHosts(),
          event.getRacks(), priority, workload, requestTimeMs);
    }

    public ContainerRequest(TaskAttemptId attemptID,
                            Resource capability, String[] hosts, String[] racks,
                            Priority priority, long workload) {
      this(attemptID, capability, hosts, racks, priority, workload,
          System.currentTimeMillis());
    }

    public ContainerRequest(TaskAttemptId attemptID,
        Resource capability, String[] hosts, String[] racks,
        Priority priority, long workload, long requestTimeMs) {
  ...
  protected void addContainerReq(ContainerRequest req) {
    ...
    for (String host : req.hosts) {
      ...
      if (!isNodeBlacklisted(host)) {
        addResourceRequest(req.priority, host, req.capability, req.workload);
      }      
    }

    // Nothing Rack-local for now
    for (String rack : req.racks) {
      addResourceRequest(req.priority, rack, req.capability, req.workload);
    }

    // Off-switch
    addResourceRequest(req.priority, ResourceRequest.ANY, req.capability, req.workload);
  }
  ...
  private void addResourceRequest(Priority priority, String resourceName,
    ...
    if (remoteRequest == null) {
        ...
        LOG.info("remoteRequest: " + remoteRequest);
  ...
  protected ContainerRequest getFilteredContainerRequest(ContainerRequest orig) {
    ...
    ContainerRequest newReq = new ContainerRequest(orig.attemptID, orig.capability,
        hosts, orig.racks, orig.priority, orig.workload); 
    return newReq;
  }
