...
public class RMContainerAllocator extends RMContainerRequestor
  ...
  */
  ...
  protected synchronized void handleEvent(ContainerAllocatorEvent event) {
    ...
    if (event.getType() == ContainerAllocator.EventType.CONTAINER_REQ) {
      ...
      if (reqEvent.getAttemptID().getTaskId().getTaskType().equals(TaskType.MAP)) {
        ...
        TaskId ss = reqEvent.getAttemptID().getTaskId();
        wl = workloadcalculation(ss);
        if (reqEvent.getEarlierAttemptFailed()) {
          //add to the front of queue for fail fast
          pendingReduces.addFirst(new ContainerRequest(reqEvent, PRIORITY_REDUCE,wl));
        } else {
          pendingReduces.add(new ContainerRequest(reqEvent, PRIORITY_REDUCE,wl));
  ...
  public long workloadcalculation(TaskId id){
      	...
      	time = scheduledRequests.maps.size() * (a + b) + scheduledRequests.reduces.size() * (c + d);
...
private static String getHost(String contMgrAddress) {
  ...
  class ScheduledRequests {
    ...
    public long workloadcalculation(TaskId id){
      	...
      	time = maps.size() * (a + b) + reduces.size() * (c + d);
    ...
    void addMap(ContainerRequestEvent event) {
      ...
      TaskId ss = event.getAttemptID().getTaskId();
      wl = workloadcalculation(ss);
      
      if (event.getEarlierAttemptFailed()) {
        earlierFailedMaps.add(event.getAttemptID());
        request = new ContainerRequest(event, PRIORITY_FAST_FAIL_MAP, wl);
       ...
       request = new ContainerRequest(event, PRIORITY_MAP, wl);
      }
